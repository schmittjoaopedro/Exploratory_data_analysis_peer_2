...
dmelt <- melt(filtered, c("year","fips"))
data <- dcast(dmelt, fips + year ~ variable, sum)

g <- ggplot(data, aes(x = factor(year), y = Emissions))
g + geom_bar(stat = "identity") + 
    facet_grid(. ~ fips, labeller = as_labeller(c(`24510` = "Baltimore City, MD", `06037` = "Los Angeles, CA"))) +
    xlab("Year") + 
